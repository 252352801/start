...
//关闭控件
  Dates.close = function(){
    ...
    //20170616 WeiQingYu  begin
    var laydatebox=Dates.query('#laydate_box');
    try {
      document.body.removeChild(laydatebox);
      var bodycls=document.body.className.split(/\s+/);
      var clsindex=bodycls.indexOf('laydate_body');
      if(clsindex>=0){
        bodycls.splice(clsindex,1);
        document.body.className=bodycls.join(' ');
      }
    }catch (err){

    }
    /// 20170616 WeiQingYu   end
    Dates.box=null;
    Dates.elem = null;
  };
...
//事件
  Dates.events = function(){
    ...
    Dates.on(as.oclear, 'click', function(){
      Dates.elem[as.elemv] = '';
      Dates.close();
      typeof Dates.options.choose === 'function' && Dates.options.choose('');//2017617新增by  WeiQingYu
    });
