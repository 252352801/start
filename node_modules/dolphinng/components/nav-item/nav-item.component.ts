...
export class NavItemComponent implements OnInit{
  ...
  ngOnInit() {
    ...
    this.rootElem.addEventListener('click', (ev)=> {
      ...
      if(navItems.length){//清除所有导航激活状态
        for(let i=0,len=navItems.length;i<len;i++){
          this.removeClass(navItems[i],'active');
        }
      }
      //子导航收起
      let subNavWraps:any=document.querySelectorAll('nav-item .nav-sub');
      if(subNavWraps){
        for(let i=0,len=subNavWraps.length;i<len;i++){
          subNavWraps[i].style.height='0';
        }
      }
      ...
      let subWrap:any=this.rootElem.querySelector('.nav-sub');
      if(wouldActive){
        this.addClass(this.rootElem,'active');
        if(!isAsideFolded) {
          this.openSubNav(subWrap);
        }else{
          subWrap.style.height='auto';
        }
      }
    });
  ...
  private openSubNav(subNavElem:any){
      if(subNavElem){
       // setTimeout(()=>{
          let children=subNavElem.querySelectorAll('li');
          let height=0;
          if(children) {
